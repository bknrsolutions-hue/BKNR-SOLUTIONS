...
describe('AuthContainer', () => {
  it('uses the backend landing page and ignores cross-origin auth messages', async () => {
    const handleLoginSuccess = vi.fn().mockResolvedValue(undefined);
    render(<AuthContainer handleLoginSuccess={handleLoginSuccess} />);
    const iframe = screen.getByTitle('SVBK IT Solutions Website and Login');
    expect(iframe).toHaveAttribute('src', '/auth/landing');
