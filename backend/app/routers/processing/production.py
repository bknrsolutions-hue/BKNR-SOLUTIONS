# ============================================================
# 🔥 PRODUCTION ROUTER - FIXED VERSION
# ============================================================
...
def production_page(
    ...
    stock_pool = {}
    for s in all_stock:
        key = build_stock_key(s.production_for, s.species, s.variety, s.grade, s.packing_style, s.glaze, s.freezer)
    ...
    produced_sub = (
        ...
        .group_by(Production.batch_number, Production.brand, Production.variety_name, Production.grade, Production.packing_style)
        .subquery()
    )

    q_req = (
        db.query(pending_orders, func.coalesce(produced_sub.c.total_produced, 0).label("produced_mc_count"))
        .outerjoin(produced_sub, and_(
    ...
    usage_history, final_pending_list = {}, []

    for row in requirements_data:
        r = row.pending_orders
        r.actual_produced_mc = float(row.produced_mc_count or 0)
        current_row_comp, p_spec = str(r.company_name or "").strip().upper(), str(r.species or "").strip().lower()
        p_var, p_grad, p_pack = str(r.variety or "").strip().lower(), str(r.grade or "").strip().lower(), str(r.packing_style or "").strip().lower()
        ...
        w_gl_factor = (100 - extract_number(r.weight_glaze, 0)) / 100 if extract_number(r.weight_glaze, 0) < 100 else 1.0
        ...
        usage_history[exact_key].append({"po_no": r.po_number or "N/A", "available": opening_bal, "utilized": round(r.existed_stock_util, 2), "balance": remaining_bal})
        r.util_json, stock_pool[exact_key] = json.dumps(usage_history[exact_key]), remaining_bal
        ...
        r.ref_opt_stock, ref_details = 0.0, []
        ...
        for s in all_stock:
            ...
            if (str(s.species).strip().lower() == p_spec and str(s.variety).strip().lower() == p_var and str(s.freezer or "N/A").strip().lower() == p_frz):
                if is_order_nwnc:
                    if (str(s.grade).strip().lower() == p_grad and s_gl_num == "0" and str(s.packing_style).strip().lower() != p_pack): match_ref = True
                else:
                    if (r.nw_grade != "-" and str(s.grade).strip().lower() == str(r.nw_grade).strip().lower() and s_gl_num == "0"): match_ref = True
            
            if match_ref:
                s_qty = float(s.quantity or 0) if str(s.cargo_movement_type).upper() == "IN" else -float(s.quantity or 0)
                if s_qty > 0:
                    r.ref_opt_stock += s_qty
                    ref_details.append({"po_no": f"LOC: {str(s.location or 'N/A').upper()}", "available": round(s_qty, 2), "utilized": f"AT: {str(s.production_at or 'N/A').upper()}", "balance": round(s_qty, 2)})

        r.ref_opt_stock, r.ref_json = round(r.ref_opt_stock, 2), json.dumps(ref_details)
        ...
        r.hoso_count_calc, r.req_hlso_qty, r.req_hoso_qty = 0, 0, 0
        if "HOSO" in p_var.upper():
            r.hoso_count_calc = r.net_count_calc
            if abs(r.pending_production) > 0: r.req_hoso_qty = round(abs(r.pending_production) * w_gl_factor, 2)
        else:
    ...
    rejection_data = db.query(Soaking).filter(Soaking.company_id == company_code, Soaking.rejection_qty > 0, Soaking.status != 'Completed').all()
    soaking_monitor = db.query(Soaking).filter(Soaking.company_id == company_code, Soaking.status != 'Completed', Soaking.in_qty > 0).order_by(Soaking.date.asc(), Soaking.sintex_number.asc()).all()
    batches_with_company = [{"batch_number": g.batch_number, "production_for": g.production_for} for g in db.query(GateEntry).filter(GateEntry.company_id == company_code).order_by(GateEntry.id.desc()).all() if g.batch_number]

    start, end = get_today_range()
    today_data = db.query(Production).filter(Production.company_id == company_code, Production.date >= start.date(), Production.date <= end.date()).order_by(Production.id.desc()).all()
    edit_data = db.query(Production).filter(Production.id == edit_id, Production.company_id == company_code).first() if edit_id else None

    # FIXED: request as first argument for TemplateResponse
    return templates.TemplateResponse(
        request, "processing/production.html",
        {
            "batches_with_company": batches_with_company,
            **get_common_data(db, company_code),
...
# ... [API methods: update_soaking_status, complete_rejection remain the same] ...

@router.post("/production")
def save_production(
    request: Request, batch_number: str = Form(...), brand: str = Form(...),
    variety_name: str = Form(...), glaze: str = Form(""), freezer: str = Form(""),
    packing_style: str = Form(...), grade: str = Form(""), species: str = Form(...),
    no_of_mc: int = Form(0), loose: int = Form(0), production_qty: float = Form(0.0),
    production_type: str = Form(""), production_at: str = Form(""),
    production_for: str = Form(...), db: Session = Depends(get_db)
):
    try:
        company_code, email = request.session.get("company_code"), request.session.get("email")
        if not company_code or not email: return RedirectResponse("/auth/login", status_code=302)
        obj = Production(
            batch_number=batch_number, brand=brand, variety_name=variety_name, glaze=glaze,
            freezer=freezer, packing_style=packing_style, grade=grade, species=species,
            no_of_mc=no_of_mc, loose=loose, production_qty=production_qty,
            production_type=production_type, production_at=production_at, production_for=production_for,
            company_id=company_code, email=email, date=datetime.now().date(), time=datetime.now().time()
        )
        db.add(obj)
        db.commit()
...
def update_production(
    id: int, request: Request, batch_number: str = Form(...), brand: str = Form(...),
    variety_name: str = Form(...), glaze: str = Form(""), freezer: str = Form(""),
    packing_style: str = Form(...), grade: str = Form(""), species: str = Form(...),
    no_of_mc: int = Form(0), loose: int = Form(0), production_qty: float = Form(0.0),
    production_type: str = Form(""), production_at: str = Form(""),
    ...
    try:
        ...
        if entry:
            entry.batch_number, entry.brand, entry.variety_name = batch_number, brand, variety_name
            entry.glaze, entry.freezer, entry.packing_style = glaze, freezer, packing_style
            entry.grade, entry.species, entry.no_of_mc = grade, species, no_of_mc
            entry.loose, entry.production_qty, entry.production_type = loose, production_qty, production_type
            entry.production_at, entry.production_for = production_at, production_for
            db.commit()
