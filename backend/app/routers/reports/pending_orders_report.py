...
def pending_orders_report_page(
    ...
    for r in rows:
        ...
        # HOSO/HLSO Raw Material Requirements
        r.hoso_count_calc = 0
        r.req_hlso_qty = 0
        r.req_hoso_qty = 0
    ...
    return request.app.state.templates.TemplateResponse(
        ...
        "reports/pending_orders_report.html",
        {
            ...
            "to_date": to_date or "",
            "f_po": sorted(list({r.po_number for r in rows if r.po_number})),
            "f_grades": sorted(list({r.grade for r in rows if r.grade})),
            "f_nw_grades": sorted(list({r.nw_grade for r in rows if r.nw_grade and r.nw_grade != '-'})),
            "f_varieties": sorted(list({r.variety for r in rows if r.variety})),
            "f_glazes": sorted(list({r.count_glaze for r in rows if r.count_glaze})),
            "f_species": sorted(list({r.species for r in rows if r.species})),
            "is_admin": role == "admin"
        }
    )