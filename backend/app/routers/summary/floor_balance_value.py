# ============================================================
# FLOOR BALANCE VALUE ROUTER (FINAL SYNCED VERSION)
# ============================================================
...
def calculate_balance_value(db: Session, company_id: str, batch: str, variety: str, count: str, species: str, qty: float, source_type: str):
    ...
    # 1. SOURCE BASED RATE CALCULATION (Syncing with Inventory Costing Logic)
    if source_type == "RMP":
        rmp_stats = db.query(
        ...
        avg_rate = float(rep_stats.ta / rep_stats.tq) if rep_stats and rep_stats.tq and rep_stats.tq > 0 else 0.0

    # 2. HOSO Case (Direct Value)
    if variety and "HOSO" in variety.upper():
        return round(qty * avg_rate, 2)

    # 3. YIELD CALCULATION (No Soaking, No Add-ons - Pure Division)
...
def floor_balance_value_report(request: Request, db: Session = Depends(get_db)):
    ...
    if not company_id: return RedirectResponse("/auth/login", status_code=303)
