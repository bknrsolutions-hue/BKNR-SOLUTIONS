...
async def get_processing_summary(
    ...
    card = {
        "supplier_name": "N/A",
        "purchasing_location": "N/A",
        "receiving_center": "N/A",
        "vehicle_number": "N/A",
        "total_boxes": 0,
        "challan_number": "N/A",
        "gate_pass_number": "N/A",
        "rmp_qty": 0, "rmp_amount": 0,
        "reprocess_qty": 0, "reprocess_amount": 0,
        "deheading_qty": 0, "deheading_amount": 0,
        "peeling_qty": 0, "peeling_amount": 0,
        "soaking_qty": 0,
        "chemical_qty": 0,
        "salt_qty": 0,
        "production_qty": 0,
        "stock_qty": 0, "stock_amount": 0,
        "floor_qty": 0,
        "grading_qty": 0
    }
    ...
    # 3. Fetch Data & Calculate Card Totals
    if batch and company_code:
        ...
        if prod_type == "RMP":
            ...
            # Map Gate Entry details to Card
            if rows["gate"]:
                g = rows["gate"][0]
                card.update({
                    "supplier_name": g.supplier_name,
                    "purchasing_location": g.purchasing_location,
                    "receiving_center": g.receiving_center,
                    "vehicle_number": g.vehicle_number,
                    "challan_number": g.challan_number,
                    "gate_pass_number": g.gate_pass_number,
                    "total_boxes": sum(row.no_of_material_boxes or 0 for row in rows["gate"])
                })

            card["rmp_qty"] = sum(r.received_qty or 0 for r in rows["rmp"])
            card["rmp_amount"] = sum(r.amount or 0 for r in rows["rmp"])
            ...
            card["grading_qty"] = sum(g[4] or 0 for g in grad_items)
            for g in grad_items: 
            ...
            if rows["reprocess"]:
                ...
                card.update({
                    ...
                    "total_boxes": sum(row.no_of_mc or 0 for row in rows["reprocess"])
                })
            
            card["reprocess_qty"] = sum(r.in_qty or 0 for r in rows["reprocess"])
            card["reprocess_amount"] = sum(r.inventory_value or 0 for r in rows["reprocess"])
            
            for r in rows["reprocess"]: 
                combos.add((r.grade, r.species, r.production_at, r.variety, "REPROCESS"))

        # C. Stage Totals
        card["deheading_qty"] = sum(d.hlso_qty or 0 for d in rows["deheading"])
        card["deheading_amount"] = sum(d.amount or 0 for d in rows["deheading"])
        card["peeling_qty"] = sum(p.peeled_qty or 0 for p in rows["peeling"])
        card["peeling_amount"] = sum(p.amount or 0 for p in rows["peeling"])
        card["soaking_qty"] = sum(s.in_qty or 0 for s in rows["soaking"])
        card["chemical_qty"] = sum(s.chemical_qty or 0 for s in rows["soaking"])
        card["salt_qty"] = sum(s.salt_qty or 0 for s in rows["soaking"])
        card["production_qty"] = sum(pr.production_qty or 0 for pr in rows["production"])
        
        # FIXED: Stock Qty and Inventory Value calculation
        card["stock_qty"] = sum(st.quantity or 0 for st in rows["stock"])
        card["stock_amount"] = sum(st.inventory_value or 0 for st in rows["stock"])

        # D. Floor Balance Calculation
        total_floor = 0
    ...
    return templates.TemplateResponse("summary/processing_summary.html", {
        "request": request,
        "companies": companies,
        "batches": batches,
