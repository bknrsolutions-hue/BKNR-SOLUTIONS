...
def get_glaze_factor(glaze_str: str) -> float:
    """Glaze % ని బట్టి క్వాంటిటీ అడ్జస్ట్‌మెంట్ ఫ్యాక్టర్ ఇస్తుంది"""
...
def get_process_addon(variety: str, freezer: str, purpose: str) -> int:
    """Process మరియు Freezer ని బట్టి అదనపు చార్జీలు"""
    v = str(variety).upper()
    f = str(freezer).upper()
    p = str(purpose).upper()
...
def inventory_costing_page(
    ...
    # 1. FETCH DATA
    inventory_rows = db.query(stock_entry).filter(stock_entry.company_id == comp_code)
    if from_date: inventory_rows = inventory_rows.filter(stock_entry.date >= date.fromisoformat(from_date))
    if to_date: inventory_rows = inventory_rows.filter(stock_entry.date <= date.fromisoformat(to_date))
    ...
    for batch in batch_numbers:
        ...
        total_rmp_amt = db.query(func.sum(RawMaterialPurchasing.amount)).filter(
            RawMaterialPurchasing.company_id == comp_code, RawMaterialPurchasing.batch_number == batch
        ).scalar() or 0

        # Floor Value Deduction
        total_floor_val = 0
        combos = db.query(
    ...
    for r in inventory_rows:
        ...
        # Raw Weight Equivalent for this item
    ...
    for r in inventory_rows:
        ...
        # Add-ons
        addon_cost = get_process_addon(r.variety, r.freezer, r.purpose or "N/A")
