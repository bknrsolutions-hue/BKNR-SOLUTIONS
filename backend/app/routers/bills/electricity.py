...
async def get_all_electricity_audit(request: Request, db: Session = Depends(get_db)):
    ...
    return [{
        ...
        "invoice_no": l.production_at, # Location Unit string mapped into token reference
...
def export_electricity_excel(request: Request, db: Session = Depends(get_db)):
    ...
    filename = f"Electricity_Consumption_Ledger_{datetime.now().strftime('%Y%m%d_%H%M%S')}.xlsx"
