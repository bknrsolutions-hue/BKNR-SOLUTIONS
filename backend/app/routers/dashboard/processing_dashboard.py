...
def processing_dashboard(
    ...
    gate_today = db.query(func.count(GateEntry.id))\
        .filter(GateEntry.company_id == company_id,
                GateEntry.date == today).scalar() or 0
    ...
    rmp_today = db.query(func.coalesce(func.sum(
        ...
        .filter(RawMaterialPurchasing.company_id == company_id,
                RawMaterialPurchasing.date == today).scalar()
    ...
    production_today = db.query(func.coalesce(func.sum(
        ...
        .filter(Production.company_id == company_id,
                Production.date == today).scalar()

    # -------------------------------------------------
    # DAY WISE FLOW (DATE RANGE)
    # -------------------------------------------------
    days = []
    cur = from_date
    while cur <= to_date:
    ...
    flow_data = {
        "gate":       [sum_for_date(GateEntry, GateEntry.id, d) for d in days],
    ...
    hourly_data = {
        ...
        "gate": hourly_sum(GateEntry, GateEntry.id),
    }

    # -------------------------------------------------
    # FLOOR BALANCE (SAME AS FLOOR BALANCE REPORT)
    # -------------------------------------------------
    combos = set()
